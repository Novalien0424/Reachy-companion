...
import reachy_companion.personality as personality_mod
from reachy_companion.config import config
from reachy_companion.avatars import (
    DEFAULT_AVATAR_FILE,
    avatar_id_for,
    read_avatar_svg,
)
from reachy_companion.personality import save_user_personality
from reachy_companion.profile_store import DEFAULT_PROFILE_NAME
...
def test_avatar_unavailable_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "reachy_companion.personality_routes.read_avatar_svg",
        lambda name: None,
    )
