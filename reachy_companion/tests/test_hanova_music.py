...
def test_both_tools_reach_the_model_session():
    ...
    names -- they are the actions of the `music` façade, which is what the
    ...
    try:
        ...
        assert "music" in registry, "the locked profile no longer lists the family"
        reachable = {tool.name for tool in type(registry["music"]).ACTIONS.values()}
        assert {"play_music", "stop_music"} <= reachable
    finally:
