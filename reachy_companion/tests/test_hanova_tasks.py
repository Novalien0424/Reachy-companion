...
def test_all_four_tools_reach_the_model_session():
    ...
    names -- they are the actions of the `tasks` façade, which is what the
    ...
    try:
        ...
        assert "tasks" in registry, "the locked profile no longer lists the family"
        reachable = {tool.name for tool in type(registry["tasks"]).ACTIONS.values()}
        assert {"task_add", "task_list", "task_complete", "task_delete"} <= reachable
    finally:
