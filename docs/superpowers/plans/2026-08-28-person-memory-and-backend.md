...
- [ ] **Step 4: Test** (follow the file's existing fake-connection pattern for extended-check tests): a late recognition with stored facts sends an `item.create` whose text contains the name and a fact text, and sets `deps.current_person`; with no facts, text `== _FACE_LATE_RECOGNITION_PROMPT.format(name=...)`. Plus the lifetime test (Codex R2-2): set `deps.current_person`, drive a session reconnect through the existing reconnect test scaffolding, assert the label is `None` before any tool could run in the new session.
...
- `backend/store.py`: JSON store at `data_dir/people.json`, photos under `data_dir/photos/<person_id>/`. Dataclasses:
  ...
  - **`updated_at` semantics** (Codex R3-3): *every* person-affecting mutation bumps `BackendPerson.updated_at` — rename, `add_fact`/`delete_fact`, `add_photo`, `set_photo_embedding`, `add_synthetic_photo`, `delete_photo`, `set_person_face_id` — so projection's top-12 ranking reflects real recency (an imported re-enrollment must not fall out of the projected set); pinned by a >12-person projection regression test.
...
**Interfaces:**
- `face_id.FaceRecognizer.embedding_for_frame(frame: NDArray[Any]) -> tuple[NDArray[np.float32] | None, Identification]` — **pure extraction of the existing `_capture` logic minus the store write and minus matching**: detect (2× decimated, existing `DETECT_DOWNSCALE`), require exactly one face (else `Identification(status="multiple_faces"/"no_face", face_count=…)`), `MIN_FACE_PX` → `too_far`, align on the full-res frame, embed; returns the raw embedding and an `Identification` describing the outcome (`status="unknown"` with `face_count=1` on success — it identifies nothing, it only extracts). `enroll` is then refactored to: `embedding_for_frame` → **`match()` against the store** (Codex R2-4 — its current return contract reports whether the sampled face already matches someone, `face_id.py:561-563`, and must be preserved) → `upsert_face`. All existing `test_face_id.py` tests must stay green — this step is a refactor, verified by the suite, plus one new direct test of the seam using the file's existing synthetic-frame fixtures **and** a regression test that enrolling a frame matching an already-enrolled person still reports that match in the returned `Identification`.
...
**Interfaces:**
- `projection.py`:
  - `project(settings, out_dir: Path) -> ProjectionResult` — writes `out_dir/faces.v1.json` **through `reachy_companion.faces` writers** (build `FaceRecord`s: per person take the newest ≤3 photos with non-None `embedding`, synthetic included; skip people with zero embedded photos for faces but still project their facts) and `out_dir/people.v1.json` **through `reachy_companion.people` writers** (`upsert_person` + `add_person_fact` against `out_dir` as the instance path — schema-exact by construction, `arcface5` stamped by the faces writer). **Facts are replayed oldest→newest** so the store's prepend-and-cap semantics leave the newest ≤20 in newest-first order (Codex R1-8). **The projected set is selected explicitly, once** (Codex R2-3): rank backend people by `updated_at` desc; faces = the top ≤12 with at least one embedding; `people.v1.json` projects **exactly those same people first** (replayed lowest-rank→highest so the store's own LRU keeps every one of them), then facts-only people fill any room left under the 12-cap. Invariant, pinned by test: every projected face record has a matching person record in the projected `people.v1.json`, including with >12 backend people mixing photo and no-photo entries. `ProjectionResult(faces_count: int, people_count: int, skipped: list[str])`.
...
- `robot.py` (all subprocess, `ssh -o BatchMode=yes`, plain `scp`, 20 s timeouts, never `expect`):
  ...
  - `robot_diff(settings) -> RobotDiff` — the *content* view for the import UI: parse fetched files via `reachy_companion.faces._read_faces_file` / the people reader; `RobotDiff(new_faces: list[RobotFace], changed_faces: list[RobotFace], new_person_facts: list[RobotPersonFacts])` with `RobotFace(record_id: str, name: str, embeddings: tuple[tuple[float, ...], ...])` and `RobotPersonFacts(name: str, face_id: str | None, facts: list[str])` — structured, identity-preserving (Codex R1-7). "New" = `record_id` not present in the backend store; "changed" (Codex R2-1) = `record_id` known but the robot record's embedding set differs from what the backend would project for that person — a voice re-enrollment into an existing name. `RobotDiff.empty` property = all three lists empty.
  - `push(settings) -> PushResult` — sequence: fetch → compute drift → **one explicit gate predicate** (Codex R3-1): `blocked = (drift or never_pushed_with_existing_robot_content) and not robot_diff(...).empty` — i.e. a push is refused exactly when the robot holds content the backend does not know (new, changed, or facts); drift whose content is fully known ("imported drift", hashes stale by design after `apply_import`) proceeds, and a never-pushed robot with absent/empty stores proceeds trivially (`PushResult(pushed=False, blocked_by=diff)` when refused); else project to a temp dir, then **guarded remote promote** (Codex R1-1): `scp` both files to `$INSTANCE_DIR/.faces.push.tmp` / `.people.push.tmp`, then ONE ssh command that (a) re-checks both current remote files still match the pre-push sha256s captured at fetch (absent file ⇒ expected-absent), and (b) `mv`s both tmp files into place — abort with a distinct exit code on mismatch, which `push` reports as a race (`blocked_by`). `mv` on the same filesystem is atomic per file; the between-files window is microseconds against an app that only ever reads — recorded as accepted residual risk in the module docstring. Then verify by re-fetching, comparing sha256 locally, parsing counts locally (no robot-python assumptions), and **only then** `set_sync_meta` with the new hashes + timestamp. `PushResult(pushed: bool, faces_count: int, people_count: int, blocked_by: object | None)`.
  - `import_from_robot(settings) -> RobotDiff` (preview) and `apply_import(settings, diff) -> ImportResult` — **name-collision rule** (Codex R3-2): a new robot face whose normalized name matches an existing backend person with **no** `face_id` is *attached* to that person (`set_person_face_id(record_id)` + synthetic photos) rather than creating a duplicate; a match against a person carrying a **different** `face_id` is a conflict — `apply_import` skips it and reports it in its return value (`ImportResult(applied: int, conflicts: list[str])`) for the UI to surface to the operator (rename one side, then re-import). Otherwise new faces become backend people (name + `face_id=record_id`) with their robot embeddings copied into **synthetic** `BackendPhoto` entries via `add_synthetic_photo` (so projection round-trips them and voice enrollments survive); **changed** faces (Codex R2-1) have the robot's current embeddings added as the person's *newest* synthetic photos, so projection's newest-≤3 rule emits exactly the robot's embedding set from then on; new facts append via `add_fact` matched by normalized name. After apply, the next `push` re-fetches and re-checks drift (import does not clear it; the push after a successful import will still see robot hashes ≠ last-pushed — `push` therefore also accepts the case where drift exists but `robot_diff(...).empty` is true, i.e. nothing on the robot — new, changed, or facts — is unknown to the backend: "imported drift", allowed to proceed; since changed faces were imported as the newest samples, that push rewrites the robot with its own embedding content, never overwriting a re-enrollment).
...
**Steps:**
- [ ] **Step 1: Scripted E2E on the Mac** (`selftest.py`, runnable via `run.sh` venv): with `data_dir` in a temp location — takes `--enroll-photo PATH` and `--probe-photo PATH`, **two different real photos of the same person** (operator-provided; synthetic faces are not acceptable): create person, embed the enroll photo, project, then load the projected `faces.v1.json` with `reachy_companion.face_id.FaceRecognizer.match` against the probe photo's embedding → `recognized` at ≥ the 0.363 threshold (Codex R2-5 — same-photo matching only proves serialization; if only one photo is available, the run is recorded as a reduced smoke test, not E2E evidence). Print counts and score; record the exact photos used in the feature row's evidence.
...
- [ ] **Step 3: DECISIONS.md** — add **D-025** recording: the PRD §9 amendment, option C architecture (Mac authoritative, robot as projection, push-guarded-by-import), the three-way greeting + 4 s wake budget, person scoping via `deps.current_person` (cleared per session — handler init and reconnect — and why a non-recognized glance does not clear it), still-pose hold, the `embedding_for_frame` seam, and the explicitly-rejected alternatives (robot-side `people.*` RPC; extending `memory.v1.json`; greeting fields in the profile schema).
...
**Round 2 (2026-08-28, 6 findings, all accepted):**

| # | Severity | Finding | Ruling |
|---|---|---|---|
| R2-1 | high | "imported drift, diff empty" could overwrite a re-enrollment into a known id (hash drifts, id-diff empty) | **Accepted** — `RobotDiff.changed_faces` (known id, embedding set differs from projection); blocks push until imported; import adds the robot embeddings as newest synthetic photos so the next push rewrites identical content |
| R2-2 | high | Task 13 D-025 wording still said "per app run", contradicting the R1-4 fix | **Accepted** — wording fixed to per-session; reconnect-clears-label test added to Task 3 |
| R2-3 | medium | people.v1.json projection through store LRU could evict a face-projected person past 12 | **Accepted** — projected set selected explicitly once; invariant test: every projected face has its person record |
| R2-4 | medium | `embedding_for_frame` refactor would drop `enroll()`'s already-matches return contract (face_id.py:561-563) | **Accepted** — enroll = extract → match → upsert; regression test added |
| R2-5 | medium | selftest matched a photo against itself — serialization proof, not recognition | **Accepted** — `--enroll-photo` / `--probe-photo`, two distinct photos; single-photo runs labeled reduced smoke |
| R2-6 | medium | empty-after-normalization names/facts not rejected | **Accepted** — `ValueError` in store, 400 in API, tests |

**Round 3 (2026-08-28, 3 findings, all accepted — review closed):**

| # | Severity | Finding | Ruling |
|---|---|---|---|
| R3-1 | high | push gate text contradicted the imported-drift rule (could block forever after import) | **Accepted** — single explicit predicate: blocked iff the robot holds content unknown to the backend (`not robot_diff(...).empty`) |
| R3-2 | high | robot-enrolled name colliding with an existing backend person breaks import (unique-name constraint) | **Accepted** — attach when the backend person has no `face_id`; explicit operator-surfaced conflict when it has a different one; `ImportResult(applied, conflicts)` |
| R3-3 | medium | `updated_at` bump semantics unspecified → imported re-enrollment could fall out of the top-12 projection | **Accepted** — every person-affecting mutation bumps `updated_at`; >12-person regression test |

Review complete: 3 rounds, 22 findings, 20 accepted, 2 partially accepted (R1-4, R1-9) with recorded reasons, 0 rejected outright.
