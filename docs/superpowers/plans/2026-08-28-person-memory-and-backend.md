...
- [ ] **Step 7: `.env.example`.** Update the FACE block: `# FACE_WAKE_BUDGET_MS=4000`, `# FACE_WAKE_ATTEMPTS=5`, add `# FACE_GREETING_FACTS=6` with one comment line each ("boot waits up to this long to see a familiar face before greeting"; "how many remembered facts a personalized greeting may mention, 0 disables"). Run `tests/test_openai_realtime_config.py` — the env-docs test must pass.
...
- [ ] **Step 3:** In the handler `__init__` beside `self._user_has_spoken = False` (line 423): `self.deps.current_person = None` — one comment line: `# Person-scoped memory label (spec §3.3): set on recognition, reset per app run.` (Deliberate deviation from spec's "cleared on session close": a websocket reconnect does not change who is standing there; the label resets per app run.)
...
- `backend/store.py`: JSON store at `data_dir/people.json`, photos under `data_dir/photos/<person_id>/`. Dataclasses:
  ...
  - `BackendPhoto(id: str, filename: str, added_at: int, embedding: tuple[float, ...] | None, error: str | None)` — `embedding` is the 128-float SFace vector or None when embedding failed; `error` one of `"no_face" | "multiple_faces" | "too_far" | "decode_failed" | "internal_error"` or None.
  ...
  - Functions: `list_people(settings)`, `get_person(settings, person_id)`, `create_person(settings, name) -> BackendPerson`, `rename_person`, `delete_person` (also removes its photo dir), `add_fact(settings, person_id, text)`, `delete_fact(settings, person_id, fact_id)`, `add_photo(settings, person_id, filename, raw: bytes) -> BackendPhoto` (saves bytes, embedding filled by Task 9's caller via `set_photo_embedding(settings, person_id, photo_id, embedding, error)`), `delete_photo`, `set_person_face_id`.
  - Same atomic tmp+replace write idiom as `people.py`; ids `bp_`/`bf_`/`bph_` prefixed. **No caps here** — the Mac holds everything; the 12/3 caps are a projection concern (Task 10). Name uniqueness enforced case-insensitively (`create_person` raises `ValueError` on duplicate).
...
- [ ] **Step 1: Failing tests** — round-trip create/rename/delete person; duplicate name raises; add/delete fact; add photo stores bytes on disk and lists back; delete person removes the photo dir; corrupt `people.json` reads as empty list (log + `[]`, the robot-store idiom).
...
**Interfaces:**
- `projection.py`:
  - `project(settings, out_dir: Path) -> ProjectionResult` — writes `out_dir/faces.v1.json` **through `reachy_companion.faces` writers** (build `FaceRecord`s: per person take the newest ≤3 photos with non-None `embedding`; skip people with zero embedded photos for faces but still project their facts) and `out_dir/people.v1.json` **through `reachy_companion.people` writers** (`upsert_person` + `add_person_fact` against `out_dir` as the instance path — schema-exact by construction, `arcface5` stamped by the faces writer). People ranked by `updated_at` desc; only the top 12 with embeddings project into faces (the robot cap); `ProjectionResult(faces_count: int, people_count: int, skipped: list[str])`.
  - Robot faces records preserve `face_id` when the backend person has one (so ids stay stable across pushes): build `FaceRecord(id=person.face_id or fresh, …)` — add a tiny helper in `faces.py` ONLY if record construction from raw floats needs one; prefer constructing `FaceRecord` directly and calling `faces._write_faces_file` (it is module-private by underscore but same-repo use is deliberate; add a one-line comment).
- `robot.py` (all subprocess, `ssh -o BatchMode=yes`, plain `scp`, 20 s timeouts, never `expect`):
  ...
  - `diff_unknown_records(settings, robot_faces: Path | None, robot_people: Path | None) -> RobotDiff` — parse via `reachy_companion.faces._read_faces_file` / `people` reader; `RobotDiff(unknown_faces: list[str], unknown_person_facts: dict[str, list[str]])` = records/facts on the robot whose id (faces) or (name, fact text) (people) the backend store does not contain.
  - `push(settings) -> PushResult` — fetch → diff → **refuse when the diff is non-empty** (`PushResult(pushed=False, blocked_by=diff)`); else project to a temp dir, scp both files, then verify: ssh `shasum -a 256` matches local, and ssh `python3 -c` count check is NOT used (no assumptions about robot python env beyond shasum — count verification happens by re-fetching and parsing locally). `PushResult(pushed: bool, faces_count: int, people_count: int, blocked_by: RobotDiff | None)`.
  - `import_from_robot(settings) -> RobotDiff` (preview) and `apply_import(settings, diff) -> None` — unknown faces become backend people (name + `face_id`, no photos, embeddings NOT imported — the robot's embeddings stay robot-side; the person is marked `face_id`-linked so pushes preserve the robot record: `apply_import` also stores the robot's embeddings? **No.** Decision: to keep the Mac authoritative *and* not lose voice enrollments, `apply_import` copies the robot `FaceRecord` embeddings into synthetic `BackendPhoto` entries (`filename="<voice-enrolled>"`, no bytes on disk, `embedding` filled) so projection round-trips them. Unknown person facts append via `add_fact`.)
  ...
  - Tests mock `subprocess.run` and `httpx` (respx is not installed — monkeypatch a thin `_http_post/_http_get` seam instead).

- [ ] **Step 1:** Failing tests: projection produces files that `reachy_companion.faces._read_faces_file` / people reader load back with the right counts, `arcface5` markers, ≤3 embeddings, ≤12 people, stable `face_id` preservation, facts projected for a person with no photos; push blocked when fake-fetched robot store has an unknown record id; push proceeds (subprocess calls recorded in order: scp faces, scp people, ssh shasum×2) when diff empty; import preview lists a voice-enrolled robot person; apply_import creates the synthetic-photo person and re-projection round-trips its embedding.
...
| `GET /api/people/{id}/photos/{photo_id}/file` | the image bytes (for thumbnails) |
| `POST /api/people/{id}/facts` | `{text}` → fact |
| `DELETE /api/people/{id}/facts/{fact_id}` | — |
| `GET /api/sync/status` | `{last_push_at, robot_reachable, drift}` (drift = fetch + diff, non-blocking failures reported as `robot_reachable: false`) |
...
- [ ] **Step 5:** Full gates once more (robot suite + backend suite + ruff + mypy). Commit: `docs: person-memory wave state, D-025, verification rows`.
...
(Codex findings and accept/reject decisions recorded here per project contract.)
